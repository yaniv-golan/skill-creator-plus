...
def validate_skill(skill_path):
    ...
    # Per the agentskills spec (and this skill's own checklist), name must
    # match the skill folder name — installers key on the directory.
    folder_name = Path(skill_path).resolve().name
    if name != folder_name:
        return False, (
            f"Name '{name}' must match the skill folder name '{folder_name}'"
        )
    ...
    if not description:
        return False, "Description cannot be empty or whitespace-only"
